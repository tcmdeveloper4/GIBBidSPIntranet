...
export default class ValidationFormWebPart extends BaseClientSideWebPart<{}> {
  ...
  public render(): void {
     // Extract the TenderID from the URL
  const urlParams = new URLSearchParams(window.location.search); // Local variable, not a class member
  const tenderID = urlParams.get('TenderID'); // Assuming the URL has a parameter 'TenderID'
    this.domElement.innerHTML = `
<div class="${styles['form-container']}">
    <h1>Validation form for ${tenderID ? tenderID : 'Unknown Tender'}</h1>
...
// Get the tenderID from the URL
    const urlParams = new URLSearchParams(window.location.search);
  const tenderID = urlParams.get('TenderID'); // Assuming TenderID is the query parameter
